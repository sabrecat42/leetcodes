    ...
    if (reg_len>=2 && reg[1] == '*') {                 // when star
        ...
        if (reg_len>=4) {
            ...
            if (reg[0]=='.') {
                ...
                for (int i = 1; i < reg_len/2; i++) {
                    if (reg[i*2]=='.' && reg[i*2+1]=='*') use_wildcard_star = true;
                    if ((reg[i*2]==reg[0] || use_wildcard_star) && reg[i*2+1]=='*') {
                        add_to_reg+=2;
                    } else {
                        break;
                    }
        ...
        if (reg[0]=='.' || use_wildcard_star) {                              // when wildcard
